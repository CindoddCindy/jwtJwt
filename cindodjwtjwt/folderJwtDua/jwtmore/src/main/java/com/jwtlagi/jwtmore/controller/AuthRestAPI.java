...
public class AuthRestAPI {
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginForm loginRequest) {
        ...
        Authentication authentication = authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(
                        loginRequest.getNameUser(),
                        loginRequest.getPasswordUser()
                )
        );
    ...
    public ResponseEntity<String> registerUser(@Valid @RequestBody SignUpForm signUpRequest) {
        ...
        User user = new User(signUpRequest.getNameUser(), signUpRequest.getEmailUser(),
                signUpRequest.getPhoneUser(), encoder.encode(signUpRequest.getPasswordUser()));
        ...
        strRoles.forEach(role -> {
            switch(role) {
                case "admin":
                    Role adminRole = roleRepository.findByName(RoleName.ROLE_USER_SELLER)
                            .orElseThrow(() -> new RuntimeException("Fail! -> Cause: User Role not find."));
                    roles.add(adminRole);

                    break;
                case "pm":
                    Role pmRole = roleRepository.findByName(RoleName.ROLE_USER_BUYER)
                            .orElseThrow(() -> new RuntimeException("Fail! -> Cause: User Role not find."));
                    roles.add(pmRole);
